...
export default class Form {
	...
	bind() {
		...
		$('.form_policy').on('click', (e) => {
			let $el = $(e.currentTarget);

			if ($el.hasClass('_active')) {
				$el.siblings('button.form_submit_button').addClass('disabled');
			} else {
				$el.siblings('button.form_submit_button').removeClass('disabled');
			}
		})
		...
		$('body').on('click', () => {
			if (!this.$formWrap.find('[data-success]').hasClass('_hide')) {
				this.$formWrap.find('[data-success]').addClass('_hide');
			}
		});
	...
	checkValid() {
		this.$submitButton.removeClass('disabled');
		if (this.$form.find('.form_input_invalid').length > 0 || !this.$form.find('.form_policy').hasClass('_active')) {
			this.$submitButton.addClass('disabled');
		}
	}
	...
	success(data, formType) {
		...
		// this.$formWrap.find('[data-success] [data-success-phone]').text(formData.get('phone'));

		// this.$formWrap.find('[data-success] [data-success-phone]').text(this.$formWrap.find('input[name="phone"]').val());
